...
export const resolveBusinessPlanProductId = (
  ...
  if (!prefix || !prefix.trim()) {
    throw new Error(
      "IAP product mapping is missing. Set EXPO_PUBLIC_IAP_BUSINESS_PLAN_PREFIX or return app_store_product_id from API.",
    );
  }
...
export const purchaseAndVerifyIosIap = async (params: {
  ...
  const { productId, kind, referenceId } = params;
