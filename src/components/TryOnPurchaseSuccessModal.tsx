...
interface TryOnPurchaseSuccessModalProps {
  visible: boolean;
  /** explore: navigate CTA + skip; tools: single continue that only closes */
  source?: "explore" | "tools" | null;
  onTryNow: () => void;
  onSkip: () => void;
  onContinue: () => void;
}

export default function TryOnPurchaseSuccessModal({
  visible,
  source = "explore",
  onTryNow,
  onSkip,
  onContinue,
}: TryOnPurchaseSuccessModalProps) {
  const { colors } = useTheme();
  const theme = colors as Theme;
  const styles = useMemo(() => createStyles(theme), [colors]);
  const { t } = useTranslation();
  const isFromTools = source === "tools";
  ...
  return (
    <Modal
      ...
      onRequestClose={isFromTools ? onContinue : onSkip}
    >
      <View style={styles.modalOverlay}>
        ...
        >
          ...
          <Text style={styles.subtitle}>
            {t("tryOnPurchaseSuccessSubtitle")}
          </Text>

          <Text style={styles.message}>{t("tryOnPurchaseSuccessMessage")}</Text>
